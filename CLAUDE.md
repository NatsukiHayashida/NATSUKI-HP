...
**外せないこと（これを崩すと、嘘になるか読めなくなる）**

- **一つの図で伝える結論は一つだけ**。図の直前の見出しで結論を言い切り、直後は一文で止める
- **事実の関係は記録どおりにする**。何が何に接するか・入るか・またぐか、どちらが大きいか。
  数値・個数を根拠なく描かない（例：粒を10個並べて割合を表す）
- **SVGの中に文字を入れない**。viewBoxの縮尺で潰れる。名前はHTMLで上に重ねる
  （`Callouts` / `Overlay`。位置は％指定、`bg-background` で線を切る）。
  番号だけ打って名称を凡例へ回すのは避ける（図と凡例を往復させることになる）
- **数値が結論なら `Readout` で図の直近に大きく出す**。凡例の中に埋めない
- **ライト／ダークの両方で確認する**。決め打ちの色は使わず、`currentColor` /
  `hsl(var(--background))` / `hsl(var(--primary))` の3役で組む
- **伏字ルールは図の中でも同じ**（社名・製品名・図番・金額・社内パス）

**比率は誇張してよい。模式図は縮尺図ではない（2026-08-11 追記）**

`mask-placement` を実測どおりの比率で描いたら、**分かりにくい図になった**。
外周の帯は実測で短辺の6.7%しかなく、そのまま描くと「帯が欠陥を食べる」という
その図の全部が小さすぎて見えない。4倍に拡大しないと差が分からなかった。

- **守るのは関係**（接する・入る・またぐ・大小）。**比率は読みやすさを優先して誇張する**
- 誇張したことと、その理由は図のコンポーネント冒頭のコメントに書く
- 「事実を捏造しない」と「実測の比率を再現する」は別物。混同しない

**既定値（理由があれば外してよい。外すならコメントに理由を書く）**

ここは好みの問題であって、守らないと不採用になる類のものではない。
**制約を積み過ぎると、安全側に倒れて丸と四角しか残らない**（実際そうなった）。

- 朱（`text-primary`）は1図につき1つの系統に寄せる。ただし
  「欠陥」と「欠陥に由来する信号」のように同じ系統なら分けなくてよい
- 罫線と細線で構成し、カード・背景色は控える
- 塗り（`fill` + `opacity`）はダークで想定より強く出る。迷ったら線かハッチング
- 本文が指しているものは**形として描いてよい**（検査画像の枠、金型の断面、封筒、フォルダ）。
  避けたいのは、**本文に無いものを装置らしく描き足す**こと。形そのものは禁止ではない
- モバイルは縦に積むだけで読める図（帯・数直線・漏斗）なら、専用構図を作らなくてよい。
  横長を縮めると潰れる図（並べて比較する構図）のときだけ別構図を作る
- トップページの製図モチーフ（`die-section.tsx`）の作法に寄せる
