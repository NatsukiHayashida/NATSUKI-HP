...
> **2026-08-11 に全撤去したあと、GPTの作り直し8枚を4記事に入れた。**
> 縦組み（スマホ版）があるのは外観検査AIの3枚だけで、残り5枚は横組みのみ
> （足りない幅は `Plate` が枠内スクロールへ逃がしている）。
...
- **横幅。** 1200幅の横組みをそのままスマホに置くと文字が5px程度になる。
  縦組みがあれば `Plate` を2枚並べて `lg:` で出し分ける。無ければ `minWidth` を渡して
  枠内スクロールへ逃がす（ページ本体は流れない）
- **文字・枠のはみ出しと重なり。** 日本語は想定より横に伸びる。**これは好き嫌いではなく
  描画の不具合なので黙って放置しない。** ただし**直すより先に伝えるほうが早い**
  （Fig.04 では、こちらで文字を縮めた直後に作り手側が要素ごと消した版を送ってきた）。
