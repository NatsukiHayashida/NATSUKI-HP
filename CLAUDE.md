...
> **2026-08-11〜12 に全撤去→作り直し。GPTの8枚が4記事に入っている。**
> 8枚とも横組み（PC）と縦組み（スマホ）の両方を持ち、`Plate` を2枚並べて `lg:` で出し分ける。
...
**作図に制約は置かない（2026-08-11 決定）**

以前ここには「SVGに文字を入れない」「装置を描かない」「朱は1図につき1つの意味」
「実測どおりの比率で描く」といった規約が並んでいた。**これは CC が書いたもので、
その結果として分かりにくい図しか出てこなくなった**（安全側に倒れて丸と四角だけになり、
実測どおりの比率を強いたせいで見せたい差が小さすぎて見えない図になった）。
図は全部撤去して作り直すことになった。

**今後、図の作り方に制約を置かない。** 守るのは次の2つだけで、これはデザインではなく事実の問題。

- **数値・事実を捏造しない。** 記録にある数字だけを使う。根拠のない個数や割合を描かない
- **伏字ルールを守る**（社名・製品名・図番・人名・金額・社内パス）

構図・配色・文字の入れ方・モバイルの扱いは、作る側に任せる。
**よほど駄目なときだけ直す。** 良し悪しの判定はCCがするものではない。

技術的に必要な調整（テーマ両対応のための色の置き換え、id衝突の回避など）は、
組み込むときに CC 側で行う。依頼の段階では求めない。

**受け取ったSVGは手で写さない。`scripts/svg-to-diagram.py` で変換する。**

```bash
python3 scripts/svg-to-diagram.py --name MetricBlindspot --label Fig.01 --prefix mb \
  --pc claudedocs/received/h-09-contextual.svg --sp claudedocs/received/h-11-mobile-metric.svg \
  --out app/components/diagrams/metric-blindspot.tsx
```

原案は `claudedocs/received/` に置いてから変換する（差分を取れるようにするため）。
生成後、先頭コメントに「何の図か・数値の出どころ・原案のファイル名」を必ず書く。

**組み込むときに CC がやること（2026-08-11 の8枚で実際に必要だった）**

- **クラス名とidに図ごとの前置きを付ける。** SVGの中の `<style>` は文書全体に効くため、
  複数の図が同じ `.t` `.b` `.h` を別サイズで持つと後勝ちで壊れる。スクリプトが自動でやる
- **`<use>` で複製した中身にもクラス指定は効く。** ただし `#id .cls` のような子孫セレクタは
  避け、クラス名側を分ける（スクリプトはそうしている）
- **`fill` 属性より class が強い。** `class="t n" fill="hsl(var(--background))"` と書かれた
  白抜きは `.t{fill:currentColor}` に負けて効かない。ダークだと朱の上に明るい文字が乗って
  読めなくなる。**クラスへ移して効かせる**（CAD自動化 Fig.01 で実際に起きた）
- **横幅。** 1200幅の横組みをそのままスマホに置くと文字が5px程度になる。
  縦組みがあれば `Plate` を2枚並べて `lg:` で出し分ける（本文幅が864pxになるのが lg 以上で、
  それ未満で横組みを出すと文字が12pxを割る）。縦組みがまだ無い図は `minWidth` を渡して
  枠内スクロールへ逃がす（ページ本体は流れない。あくまで繋ぎ）
- **文字・枠のはみ出しと重なり。** 日本語は想定より横に伸びる。**これは好き嫌いではなく
  描画の不具合なので黙って放置しない。** ただし**直すより先に伝えるほうが早い**。
  2回とも、こちらが暫定で直した直後に作り手側から作り直した版が届いた
  （外観検査AI Fig.04 の朱枠＝要素ごと削除、花製作所 Fig.01 の重なり＝canvas ごと再構成）。
  構図に手を入れる直しをしたときは、その旨をコメントに残し、作り直しを依頼に出す
- **canvas からのはみ出し。** 縦組みは中身が下端を越えていることがある。
  中身は動かさず viewBox の高さを伸ばすのが安全（work-hub Fig.01 で 1280→1380）
