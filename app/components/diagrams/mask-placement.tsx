 ...
 *
 * 幾何は実験記録の実画像を実測して決めてある（デザインの都合で動かさないこと）。
 *   検査画像 1600×1200 → 枠は 4:3
 *   帯       全周80px固定（四辺とも同じ太さ。短辺の6.7%・長辺の5.0%）
 *   部品     枠の44%を占める。下端だけが帯に食い込む（部品画素の約2.2%）
 *   欠陥     部品の下の縁にあり、帯の境界をまたぐ。マスク後は下半分が消える
 *
 * 部品を小さく中央に浮かせて描くと「帯が欠陥を消す」因果が絵の上で成立しない。
 * 過去2回の草案はそこで落ちている。
 *
...
const PC = { w: 1000, h: 560, fw: 200, fh: 150, band: 10, cx: 96, cy: 80, r: 68, hole: 26, bore: 20, pin: 5, contour: 76 }
const SM = { w: 320, h: 1720, fw: 280, fh: 210, band: 14, cx: 134, cy: 112, r: 95, hole: 36, bore: 28, pin: 7, contour: 106 }
...
const PC_DEFECT = 'M121 138c4-7 11-9 18-5-4 4-4 7-1 11-7-1-12 1-15 5-3-4-4-8-2-11z'
const SM_DEFECT = 'M168 193c6-9 16-12 25-7-6 5-6 10-2 15-9-2-17 2-21 7-4-5-5-10-2-15z'
...
export function MaskPlacement() {
  return (
    <Schematic
      ...
      note="入力側の帯は、端にあった欠陥まで塗りつぶす。欠陥が消えればその不良品の異常スコアが下がり、見逃しゼロを守るには閾値をその最低スコアまで下げるしかない。下がった分だけ、それまで良品と判定できていた製品が不良側に落ちる。背景を消すつもりの処理が、まわりまわって過検出を増やしていた。"
    >
      {/* PC：上下2段 × 左から右に3枚 */}
      ...
      <Legend
        items={[
          ...
          },
          {
            no: '03',
            title: '四隅の反応',
            body: '部品が写っていない背景に出ていた異常反応。過検出の主因はこれだった。両方の経路とも、狙いはこれを消すことにある。',
          },
          {
            no: '04',
            title: '比較の条件',
            body: 'いずれも「不良品の見逃しゼロ」を守った上での比較。この制約があるために、欠陥が1つ消えるだけで閾値が動き、過検出の数まで動く。',
          },
        ]}
      />
    </Schematic>
