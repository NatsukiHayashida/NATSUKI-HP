...
| 3 | H-02 | 外観検査AI：マスクの位置で結果が逆転 | 次はここ |
...
2. **モバイル用の縦組みを別に作る**。横組み（viewBox 1000幅）のまま縮めると、
   番号バルーンが5px以下・文字が読めなくなる。H-01/H-04とも `sm:` で出し分けている
3. **viewBoxの空き領域をそのままにしない**。図が上に寄っていると、その分だけ
   本文との間に空白が出る（H-01で3割の空白が出た。`viewBox="0 150 320 350"` で詰めた）
4. **GPTの図は意味を必ず検証する**。H-01は「旧指標が見ていた象限」を取り違えていた。
