...
**外観検査AIの記事に、GPTの作り直し3枚が入っている**（2026-08-11 第7セッション）。
制約を全部外して依頼したもので、図の中に見出し・数値・結論まで入った自己完結型。
`claudedocs/received/h-0{9,7,10}-contextual.svg` → `app/components/diagrams/`。

| | 図 | 本文の位置 |
|---|---|---|
| Fig.01 | `metric-blindspot` 全品不良判定と実モデルが同じ「検出率100%」で並ぶ | 引き継いだときの状況 |
| Fig.02 | `mask-placement` 入力に掛けると72.4%、出力に掛けると20.8% | 失敗が教えてくれたこと |
| Fig.03 | `domain-shift` 別の日の良品100枚が不良の分布の中に入る | 一番大きな壁 |

**未了：3枚とも横組み1本だけで、縦組みのスマホ版が無い。**
いまは `Plate`（`schematic.tsx`）が最低幅720pxを確保して枠内スクロールへ逃がしているが、
Fig.01 は2つの盤面を並べて比べる図なので、**スクロールしないと結論が見えない**。
スマホ版の依頼は `_handoff/` に生成済み（H-11〜H-13）。届いたら `Plate` ごと
`sm:` の出し分けに置き換える。

**組み込み時にCC側で直したもの（2点だけ）**：クラス名とidの前置き（`<style>` が文書全体に
効くため後勝ちで壊れる）と、`Plate` による最低幅の確保。構図・配色・文言は触っていない。

Fig.03 は初版で「100枚すべて誤検出」の朱枠が隣の帯に重なっていたので一度こちらで
文字を縮めたが、**作り手側から要素ごと消した版が届いた**（15:48）ので、そちらへ差し替えた。
いまサイトに出ているのは差し替え版。「不良178枚中107枚より右側」の引き出しも消えているが、
