...
ローカルでビルド・リント・表示確認まで通っているが、**まだ push していない**（15コミットが未push）。
...
2. **記事本文に模式図を差し込む仕組みを追加**。MDXに `<figure data-diagram="キー">` と
   書くと図が描画される。「自分用の開発基盤」に3点（収集の流れ／メール改善の前後／到達経路）。
   詳細は `CLAUDE.md` の「記事本文への模式図の差し込み」。
...
記事本文に差し込む**模式図**の仕組みを追加している（`app/components/schematic.tsx`）。
こちらも罫線・番号・モノスペースのラベルだけで組み、カードと背景色は使わない。
デスクトップは横並び、モバイルは縦積みに自動で組み替わる。
...
### 優先度：高
1. **push していない**。15コミットが手元に残っている。push すると Vercel で本番公開されるため、
   ユーザーの明示的な指示を待つこと。
...
| 模式図の共通部品 / 図の定義 | `app/components/schematic.tsx` / `article-diagrams.tsx` |
