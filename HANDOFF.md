...
**サイトに図は1枚もない。** 第6セッションで3枚入れたが、
「分かりにくい・使えない」という判断で**全部外した**（2026-08-11）。
...
場所と、使ってよい数字だけが書いてある（8か所）。作図規約は削除済み。

**残してあるもの（仕組み）**
- `app/components/schematic.tsx` — 共通枠。使うかどうかも自由
- `app/components/article-diagrams.tsx` — 登録表。**中身は空**
- `app/components/diagrams/` — 外した3枚のファイルは残っているが、サイトからの参照はゼロ
